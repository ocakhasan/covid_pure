...
test_files = os.listdir(VALIDATION_PATH)
test_files = [os.path.join(VALIDATION_PATH, filename) for filename in test_files]
print("training files are ", test_files)


def add_COVID_measurement_date():
    measurement = pd.read_csv(test_files[4],usecols =['person_id','measurement_date','measurement_concept_id','value_as_concept_id'])
...
def add_demographic_data(covid_measurement):
    ...
    person = pd.read_csv(test_files[7],usecols = ['person_id','gender_concept_id','year_of_birth','race_concept_id'])
...
def prediction(predictors):
    ...
    person = pd.read_csv(test_files[7],usecols =['person_id'])
    ...
    output_prob.to_csv('predictions.csv', index = False)
